...
// mode: 'multi-sheet' = 1 file, 1 sheet per survey (full detail with photos)
//       'single-sheet' = 1 file, 1 flat table with all surveys as rows + photos
export async function handleAdminExportSelected(db, body, env) {
  const { ids, mode } = body;
  ...
  // Helper: fetch photos for a visit and return base64-embedded HTML
  async function getPhotoHtml(visitId) {
    const photoRows = await queryAll(
      db,
      'SELECT * FROM visit_photos WHERE visit_id = ? ORDER BY category, id',
      [visitId]
    ...
    for (const p of photoRows) {
      ...
      if (cat !== currentCat) {
        currentCat = cat;
        html += `<div style="font-weight:bold;font-size:11px;color:#004EA2;margin:6px 0 2px">${catLabel}</div>`;
      }
      // Try to get image from R2 as base64
      try {
        const object = await env.PHOTOS.get(p.r2_key);
        if (object) {
          const blob = await object.blob();
          const buf = await blob.arrayBuffer();
          const b64 = btoa(String.fromCharCode(...new Uint8Array(buf)));
          const mime = blob.type || 'image/jpeg';
          html += `<img src="data:${mime};base64,${b64}" style="width:120px;height:120px;object-fit:cover;margin:3px;border:1px solid #ccc;border-radius:4px">`;
        } else {
          html += `<div style="display:inline-block;width:120px;height:120px;background:#eee;margin:3px;border:1px solid #ccc;border-radius:4px;text-align:center;line-height:120px;font-size:11px;color:#999">No Image</div>`;
        }
      } catch {
        html += `<div style="display:inline-block;width:120px;height:120px;background:#eee;margin:3px;border:1px solid #ccc;border-radius:4px;text-align:center;line-height:120px;font-size:11px;color:#999">No Image</div>`;
      }
  ...
  // Multi-sheet: 1 sheet per survey with full detail + photos
  if (mode === 'multi-sheet') {
    const sheets = [];
    for (let i = 0; i < rows.length; i++) {
      ...
      const photoHtml = await getPhotoHtml(row.id);
      const sheetName = `Survey ${i+1} - ${(row.shop_name || row.shop_code || '').slice(0, 20)}`;

      const table = `<table>
        <tr><th colspan="2" style="background:#004EA2;color:white;font-size:14px;text-align:center">${sheetName}</th></tr>
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">📋 ข้อมูลทั่วไป</th></tr>
        <tr><td style="font-weight:bold;width:160px">Supervisor</td><td>${esc(row.supervisor_name || '')}</td></tr>
        <tr><td style="font-weight:bold">ร้านค้า</td><td>${esc(row.shop_name || '')} (${row.shop_code || ''})</td></tr>
        <tr><td style="font-weight:bold">Customer</td><td>${esc(row.customer_name || '')}</td></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">👥 1. ข้อมูลพนักงานภายในร้าน</th></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">💰 2. ข้อมูลยอดขาย</th></tr>
        ...
        <tr><td style="font-weight:bold">%Ach.</td><td style="font-size:14px;font-weight:bold">${s2.haier_ach || '0.0%'}</td></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">📐 3. พื้นที่โชว์สินค้า Haier</th></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">🔧 4. ปัญหา / คู่แข่ง / การแก้ไข</th></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">🤝 5. เข้าพบ GM</th></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">📊 6. สรุปภาพรวมร้าน</th></tr>
        ...
        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">✍️ ลายเซ็น</th></tr>
        <tr><td>ผู้ตรวจ</td><td>${esc(sig.supervisor || '')}</td></tr>
        <tr><td>วันที่</td><td>${esc(sig.date || '')}</td></tr>
        <tr><td>GM</td><td>${esc(sig.gm || '')}</td></tr>

        <tr><th colspan="2" style="background:#e8f0fa;font-size:12px">📸 รูปภาพ</th></tr>
        <tr><td colspan="2">${photoHtml || '<span style="color:#999">ไม่มีรูปภาพ</span>'}</td></tr>
      </table>`;
    ...
    <style>
      table { border-collapse: collapse; font-size: 11px; font-family: Arial, sans-serif; margin-bottom: 20px; width: 100%; }
  ...
  // Single-sheet mode: flat table, one row per survey, with photos column
